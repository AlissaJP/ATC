...
export async function rechercherProduits(requete: string, limite?: number): Promise<Produit[]> {
  ...
  // Correction #23 — les SKU de variante non canoniques ne sont pas trouvables indépendamment (cf.
  // lib/services/produits.ts) : une seule fiche produit par groupe de variantes.
  const resultats = produits.filter(
    (p) => p.statut_publication === "publié" && !p.variante?.masque && p.nom.toLowerCase().includes(q)
  );
